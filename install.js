...
async.waterfall ([
    ...
    function (done){
        rl.question(++i + ") Enter your application's folder. (Path only relative to the current directory, no file names. Example  'MyCoolApplication'):", function(answer) {
    ...
    function (done){
        rl.question(++i + ") Enter your application's name:", function(answer) {
            app_name = answer;
            rl.close();
            done ();
        });
    ...
    function (done){
        rl.question(++i + ") Enter your application's entry point (defaults to 'start.js'):", function(answer) {
            app_entry = answer;
            rl.close();
            done ();
        });
    ...
    function (done){
        rl.question(++i + ") Please take a second to copy the ssh key from this server, listed below, and paste into your git repository service (such as github). This will authorize this server AMI to " +
    ...
    function (done){
        rl.question(++i + ") Now that your Git repository has been configured, enter the ssh url for remote access so this server can clone it:" , function(answer) {
            git_url = answer;
            if (git_url){
